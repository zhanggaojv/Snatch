...
- (void)setTitleArray:(NSArray *)titleArray {
    ...
    for (int i = 0; i < objArray.count; i++) {
        ...
        titleLabel.textColor=LightBlackLabelTextColor;
        ...
        NSString *string=[NSString stringWithFormat:@"%@",objArray[i]];
        NSString *name = [string subStringFrom:@"喜" to:@"获"];
        NSString *text= [string subStringFrom:@"得" to:@"("];
        NSString *dizhi= [string subStringFrom:@"(" to:@")"];
        NSString *str=[NSString stringWithFormat:@"恭喜%@获得%@（%@）",name,text,dizhi];
        
        NSRange rangeB = [str rangeOfString:[NSString stringWithFormat:@"%@",name]];
        NSRange rangeD = [str rangeOfString:[NSString stringWithFormat:@"%@",text]];
        NSRange rangeE = [str rangeOfString:[NSString stringWithFormat:@"%@",dizhi]];
        NSMutableAttributedString *aStr = [[NSMutableAttributedString alloc] initWithString:str];
        // 给每一部分分别设置颜色
        BOOL jud=[JudgeTheillegalCharacter JudgeTheillegalCharacter:name];
        if (jud==NO||rangeB.length<2||[name length]>20) {
            [aStr addAttribute:NSForegroundColorAttributeName value:kDefaultColor range:rangeB];
        }
       
        [aStr addAttribute:NSForegroundColorAttributeName value:[UIColor lightSeaGreen] range:rangeD];

        [aStr addAttribute:NSForegroundColorAttributeName value:[UIColor orangeColor] range:rangeE];
