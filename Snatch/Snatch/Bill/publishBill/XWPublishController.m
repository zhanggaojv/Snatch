...
//    _explainLabel.text = @"添加图片不超过9张，文字备注不超过300字";
    _explainLabel.text = [NSString stringWithFormat:@"添加图片不超过3张，文字备注不超过%d字",kMaxTextCount];
    //发布按钮颜色
...
-(void)HTTP_Upload{
    ...
    for (UIImage *smallImg in smallImageArray) {
        ...
        NSLog(@"smallImg:%@",smallImg);
        NSData *picData = UIImageJPEGRepresentation(smallImg, 0.5);
        NSMutableDictionary *Exparams = [[NSMutableDictionary alloc]init];
        //放到数组里边
        [Exparams addEntriesFromDictionary:[NSDictionary dictionaryWithObjectsAndKeys:picData,@"imageName", nil]];
        
        self.token=[GJTokenManager accessToken];
        NSString *token=[NSString stringWithFormat:@"%@",self.token];
        NSDictionary *dic=@{@"ht":@"2"};
        [CKHttpCommunicate createRequest:HTTP_Image withToken:token WithParam:dic withExParam:Exparams withMethod:POST success:^(id result) {
            NSLog(@"%@",result);NSLog(@"%@",result[@"msg"]);
            
            NSDictionary *data=result[@"data"];
            NSString*path=data[@"path"];
            _paths=[_paths stringByAppendingString:[NSString stringWithFormat:@",%@",path]];
            NSLog(@"123____%@",_paths);
            [self upload];
...
-(void)upload{
    NSString *pathStr = [_paths substringFromIndex:2];;
    NSLog(@"335_____%@",pathStr);
    NSString *token=[NSString stringWithFormat:@"%@",self.token];
    NSDictionary *dic=@{@"pid":[NSString stringWithFormat:@"%@",_pid],@"content":[NSString stringWithFormat:@"%@",_noteTextView.text],@"pic":[NSString stringWithFormat:@"%@",pathStr]};
    NSDictionary *shareDic=@{@"code":@"shared",@"data":dic};
