...
- (void)createUnloginedRequest:(HTTP_COMMAND_LIST)taskID WithParam:(id)param withMethod:(HTTPRequestMethod)method success:(void(^)(id result))success failure:(void(^)(NSError *erro))failure showHUD:(UIView *)showView
{
    ...
    /******************************************************************/
    NSLog(@"路径%@",[[NSURL URLWithString:URLString relativeToURL:_HTTPManager.baseURL] absoluteString]);
    ...
    if ([GJTokenManager hasAvalibleToken]) {
        token=[GJTokenManager accessToken];
    }
    [request addValue:token forHTTPHeaderField:@"token"];
    [request addValue:@"3" forHTTPHeaderField:@"device"];
    [request addValue:uuid forHTTPHeaderField:@"dataID"];
...
//                [MBProgressHUD showError:@"未能找到使用指定主机名的服务器" toView:window];
                [MBProgressHUD showError:@"无法连接到服务器" toView:window];
            }else{
...
+ (void)createTokenRequest:(HTTP_COMMAND_LIST)taskID
                   ...
                   showHUD:(UIView *)showView
{
    if (taskID < HTTP_METHOD_RESERVE)
    {
        [[CKHttpCommunicate sharedInstance] createUnloginedRequest:taskID withToken:token WithParam:param withMethod:method success:success failure:failure showHUD:showView];
    }
}
- (void)createUnloginedRequest:(HTTP_COMMAND_LIST)taskID withToken:(NSString *)token WithParam:(id)param withMethod:(HTTPRequestMethod)method success:(void(^)(id result))success failure:(void(^)(NSError *erro))failure showHUD:(UIView *)showView
{
...

{
    if (taskID < HTTP_METHOD_RESERVE)
    {
        [[CKHttpCommunicate sharedInstance] createUnloginedRequest:taskID withToken:token WithParam:param withExParam:Exparam  withMethod:method success:success failure:failure uploadFileProgress:uploadFileProgress];
    }
}


- (void)createUnloginedRequest:(HTTP_COMMAND_LIST)taskID withToken:(NSString *)token WithParam:(NSDictionary *)param withExParam:(NSDictionary*)Exparam  withMethod:(HTTPRequestMethod)method success:(void(^)(id result))success failure:(void(^)(NSError *erro))failure uploadFileProgress:(void(^)(NSProgress *uploadProgress))uploadFileProgress
{
    ...
    [request addValue:token forHTTPHeaderField:@"token"];
    [request addValue:NEWAPPVALUE forHTTPHeaderField:@"newapp"];
    //    [request addValue:text forHTTPHeaderField:@"CONTENT"];
    //    [request addValue:pid forHTTPHeaderField:@"PID"];
    
    
    /******************************************************************/
    request = [_HTTPManager.requestSerializer multipartFormRequestWithMethod:[self getStringForRequestType:method] URLString:URLString parameters:param constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if (Exparam) {
            for (NSString *key in [Exparam allKeys]) {
                ...
                //字典根据对应的key 取出图片data
                //                [formData appendPartWithFileData:[Exparam objectForKey:key] name:key fileName:fileName mimeType:@"image/png"];
                [formData appendPartWithFileData:[Exparam objectForKey:key] name:@"file" fileName:fileName mimeType:@"image/png"];
            }
        }
        /*
         if (Exparam) {
         for (NSInteger i = 0; i < Exparam.count; i ++) {
         UIImage *images = Exparam[i];
    ...
    //    [_HTTPManager.requestSerializer setValue:@"3" forHTTPHeaderField:@"device"];
    //    [_HTTPManager.requestSerializer setValue:text forHTTPHeaderField:@"CONTENT"];//描述内容
    //    [_HTTPManager.requestSerializer setValue:pid forHTTPHeaderField:@"PID"];//分期号
