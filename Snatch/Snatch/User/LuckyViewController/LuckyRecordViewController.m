...
@interface LuckyRecordViewController ()<UIWebViewDelegate,UIScrollViewDelegate>
...
-(void)addWebViewUI{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/user/lottery",URL_BASE]];
...
- (void) webViewDidFinishLoad:(UIWebView *)webView {
    ...
    NSString *dataStr = [webView stringByEvaluatingJavaScriptFromString:@"document.getElementsByName('/textarea')[0].value"];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if([@"bridge" isEqualToString:[url substringToIndex:6]]){
        ...
        if([code containsString:@"shaidanApp"]){//需要跳转页面
            NSString *json = array[1];
            NSDictionary *dic = [JSONStrToDict dictionaryWithJsonString:json];
            _pidStr = [dic objectForKey:@"pid"];
            NSLog(@"urlStr_______%@",_pidStr);

            
            XWPublishController *pub=[[XWPublishController alloc]init];
            pub.pid=_pidStr;
...
//                [self.web  loadRequest:request];
                if(![url isEqual:[NSURL URLWithString:@"http://gxtj01.statis.wayayaya.com:5001/gxtj01.html"]]){
